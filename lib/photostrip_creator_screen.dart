...
class _PhotostripCreatorScreenState extends State<PhotostripCreatorScreen> {
  ...
  List<PhotostripItem> _currentLayoutItems =
      List.generate(2, (index) => PhotostripItem(imagePath: null));
  ...
  Widget _buildEmojiFrame2() {
    return Stack(
      children: [
        ...
        const Positioned(
            top: 5, left: 5, child: Text("😎", style: TextStyle(fontSize: 24))),
        const Positioned(
            top: 5,
            right: 5,
            child: Text("🔥", style: TextStyle(fontSize: 24))),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(
            'Voucher: ${widget.voucherCode} ${widget.isStrukMode ? "(STRUK)" : "(NORMAL)"}',
          ...
          child: Stack(
            children: [
              ...
              if (_selectedFrameIndex == 1)
                Positioned.fill(child: _buildEmojiFrame1())
              else if (_selectedFrameIndex == 2)
                Positioned.fill(child: _buildEmojiFrame2())
              else if (_selectedFrameIndex >= 3)
                Positioned.fill(
                    child: Image.file(_customFrames[_selectedFrameIndex - 3],
                        fit: BoxFit.fill))
            ],
          ),
        ),
  ...
  Widget _build2StripSlots() => Column(
  ...
  Widget _buildDragTargetWrapper(int index, PhotostripItem item,
      {required double height}) {
    return DragTarget<Object>(
      builder: (ctx, cand, rej) => _buildSlotContent(index, item,
          isHighlighted: cand.isNotEmpty, height: height),
